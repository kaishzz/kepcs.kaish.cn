...
async function saveConfig() {
  ...
  try {
    ...
    pushToast('空服自动重启总开关已保存', 'success')
    await loadPanel()
  }
  catch (error) {
    pushToast((error as Error).message, 'error')
...
<template>
  <NCard embedded title="空服自动重启" class="console-form-card">
    ...
    <template v-else>
      ...
      <section v-if="recentRows.length" class="console-subsection">
        <div class="catalog-section-head">
          <div class="catalog-section-head__title">最近下发</div>
        </div>
